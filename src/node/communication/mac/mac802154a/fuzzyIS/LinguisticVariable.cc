...
#include "TrapezoidalTerm.h"
    ...
    void LinguisticVariable::createTerms(int number_of_terms,
        ...
        flScalar intersection = NAN; //Proportion of intersection between terms
        ...
        flScalar term_range = (max - min) / (number_of_terms - number_of_terms / 2);
        flScalar current_step = min + (1 - intersection) * term_range;
        for (int i = 0; i < number_of_terms; ++i) {
            LinguisticTerm* term = NULL;
            switch (mf) {
                case LinguisticTerm::MF_TRIANGULAR:
                    term = new TriangularTerm(final_labels.at(i),
                ...
                case LinguisticTerm::MF_SHOULDER:
                    if (i == 0 || i == number_of_terms - 1) {
                        term = new ShoulderTerm(final_labels[i],
                                current_step - (1 - intersection) * term_range,
                ...
                case LinguisticTerm::MF_TRAPEZOIDAL:
                    ...
                    term = new TrapezoidalTerm(final_labels.at(i),
                            current_step - (1 - intersection) * term_range,
                            current_step + intersection * term_range);
//                    term = new TriangularTerm(final_labels.at(i), current_step
//                            - (1 - intersection) * term_range, current_step + intersection
//                            * term_range);
                    break;
                default:
                    throw InvalidArgumentException(FL_AT, "Invalid option for membership function");
            }
            current_step += intersection * term_range;
            addTerm(term);
        }
