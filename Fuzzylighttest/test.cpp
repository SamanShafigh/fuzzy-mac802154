...
namespace fl {
    ...
    void Test::ComplexMamdani() {
        ...
        fl::InputLVar* priority = new fl::InputLVar("Priority");
        priority->addTerm(new fl::TriangularTerm("D", 1, 4));
        priority->addTerm(new fl::TriangularTerm("C", 3, 6));
        priority->addTerm(new fl::TriangularTerm("B", 5, 8));
        priority->addTerm(new fl::TriangularTerm("A", 7, 10));
        //engine.addInputLVar(priority);
        ...
        //block->addRule(new fl::MamdaniRule("if Energy is LOW and Priority is A then Power is HIGH", engine));
        ...
        //return;
        int i = 0;
        for (fl::flScalar in = 0.0, prio =0, dis = 100; in < 1; in += 0.1, dis += 20, prio++) {
            i++;
        	energy->setInput(in);
            distance->setInput(dis);
            priority->setInput(prio);
            engine.process();
            ...
            FL_LOG("Energy=" << in);
            FL_LOG("Distance=" << dis);
          //  FL_LOG("Priority=" << prio);
            FL_LOG("Energy is " << power->fuzzify(in));
            FL_LOG("Distance is " << distance->fuzzify(dis));
            //FL_LOG("Priority is " << priority->fuzzify(prio));
            FL_LOG("Health=" << out);
